...
public class DownloadingScalaToolChain implements ScalaToolChainInternal {
    ...
    public DownloadingScalaToolChain(File gradleUserHomeDir, WorkerDaemonFactory workerDaemonFactory, ConfigurationContainer configurationContainer, DependencyHandler dependencyHandler, FileResolver fileResolver) {
    ...
    public ToolProvider select(ScalaPlatform targetPlatform) {
        try {
            ...
            return new DefaultScalaToolProvider(gradleUserHomeDir, workerDaemonFactory, fileResolver, resolvedScalaClasspath, resolvedZincClasspath);
