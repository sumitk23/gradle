...
In addition, IntelliJ IDEA or Android Studio might spawns up to 3 Gradle daemons when editing scripts; one per each type of script: project scripts, settings scripts and initialization scripts.
...
For all other containers, accessors for elements are of type `NamedDomainObjectProvider<T>` providing the same.
...
In multi-project builds, and most Android builds are, one needs to take special care if applying the same plugin to multiple sub projects.
In that case, always request the plugin without applying it on your root project first.
This ensure that all sub projects using that plugin will load it from the same class loader, allowing for example to cross-configure projects.
...
For example, Gradle core contributes `configurations` which is a container of `Configuration` objects and `tasks` which is a container of `Task` objects ; core plugins such as `java` or `distribution` contribute containers such as `sourceSets` or `distributions`.
Community plugins also contribute containers, such as `android.buildTypes` by the `android` plugin.
...
The Gradle Kotlin DSL provides with uniform ways of interacting with containers.
...
All the containers implement `NamedDomainObjectContainer<DomainObjectType>`, some of them can contain objects of different types and implement `PolymorphicDomainObjectContainer<BaseType>`. The simplest way to interact with containers is through their API.
